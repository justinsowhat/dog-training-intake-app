...
class EnvironmentalSection(BaseModel):
    ...
    appetite_description: str = Field(..., description="How predictably does your animal eat? Appetite level?")
...
class ComprehensiveIntakeSchema(BaseModel):
    ...
    other_species_description: Optional[str] = Field(
        None, description="The specific animal when species is 'Other' (e.g., 'Ferret')."
    )
    ...
    @model_validator(mode="after")
    def validate_other_species(self) -> "ComprehensiveIntakeSchema":
        """Require a description when species is 'Other'; clear it otherwise."""
        if self.species == SpeciesEnum.OTHER:
            if not self.other_species_description or not self.other_species_description.strip():
                raise ValueError("When species is 'Other', other_species_description is required.")
        else:
            self.other_species_description = None
        return self