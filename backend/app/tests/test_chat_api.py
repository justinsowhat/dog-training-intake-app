...
async def test_suggestions_endpoint_returns_generated_chips(client, fake_llm, consultation_id):
    fake_llm.completion = completion_with_tool(
        json.dumps({"suggestions": ["On walks", "At home too", "Something else"]}),
        name="suggest_openers",
    )
    resp = await client.post(f"/consultation/{consultation_id}/suggestions")
    assert resp.status_code == 200
    assert resp.json()["suggestions"] == ["On walks", "At home too", "Something else"]


async def test_suggestions_404_for_unknown_consultation(client, fake_llm):
    resp = await client.post("/consultation/nope/suggestions")
...
async def test_chat_autonomously_proposes_and_persists_plan(client, fake_llm, consultation_id, session_maker):
    ...
    assert events[1]["plan"]["pet_name"] == "Rex"
    ...
    async with session_maker() as db:
        ...
        assert plan_row.plan_payload["pet_name"] == "Rex"
